...
class MLP2(LasagnePowered, Serializable):
    def __init__(self, output_sizes, hidden_sizes, hidden_nonlinearity,
                 output_nonlinearity, hidden_W_init=LI.GlorotUniform(), hidden_b_init=LI.Constant(0.),
                 output_W_init=LI.GlorotUniform(), output_b_init=LI.Constant(0.),
                 name=None, input_var=None, input_layer=None, input_shape=None, batch_norm=False):
        ...
        for idx, output_size in enumerate(output_sizes):
            l_out_c = L.DenseLayer(
                l_hid,
                num_units=output_size,
                nonlinearity=output_nonlinearity,
                name="%soutput_%d" % (prefix,idx),
                W=output_W_init,
                b=output_b_init,
            )
            l_outs.append(l_out_c)
