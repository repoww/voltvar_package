...
class Product(Space):
    ...
    def __init__(self, *components):
        ...
        self._comp_dim = [c.flat_dim for c in self._components]
        self._comp_range = [c.flat_dim - 1 for c in self._components]
        self._comp_base = [c / 2 for c in self._comp_range]
    ...
    def weighted_sample(self, weights):
        return [special.weighted_sample(weights[s:s + n], range(n)) for s, n, b in
                zip(self._slice, self._comp_dim, self._comp_base)]

    def determined_sample(self, weights):
        return [np.argmax(weights[s:s + n]) for s, n, b in
                zip(self._slice, self._comp_dim, self._comp_base)]

    def weighted_sample_normalized(self, weights):
        return [(special.weighted_sample(weights[s:s + n], range(n)) - b) / b for s, n, b in
                zip(self._slice, self._comp_dim, self._comp_base)]

    def determined_sample_normalized(self, weights):
        return [(np.argmax(weights[s:s + n]) - b) / b for s, n, b in
                zip(self._slice, self._comp_dim, self._comp_base)]
