...
class VoltVarEnv(Env,Serializable):
        ...
        def action_space(self):
                return Product([Discrete(11), Discrete(11),  Discrete(11), Discrete(11),
                                Discrete(2), Discrete(2), Discrete(2), Discrete(2)])

        def reset(self):
                self.state = np.zeros(1 + 8 + 1)
                if self.mode == 'Train':
                ...
                # one-hot day
                # current t
                self.state[1:9] = [0,0,0,0,-1.0,-1.0,-1.0,-1.0]
                self.state[9] = self.local_time
                return self.state
        ...
        def step(self, action):
                ...
                info = {}
                info['gain'] = - (total_loss*5*40+0.1*np.sum(np.round(np.abs(action-self.state[1:9])*self.base)))  ## bus4: 10; bus123: 5
                info['cost'] = 0.2*(min([np.sum(Vi<0.95)+np.sum(Vi>1.05), 20]))
                ...
                if self.mode == 'Train':
                        ...
                        self.state[:1] = self.load_test[self.global_time]
                self.state[1:9] = action # current tap
                self.state[9] = self.local_time
