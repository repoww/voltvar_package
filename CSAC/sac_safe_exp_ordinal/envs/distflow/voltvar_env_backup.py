...
class VoltVarEnv(Env,Serializable):
        ...
        def action_space(self):
                return Product([Discrete(11), Discrete(11), Discrete(2)])
        ...
        def step(self, action):
                ...
                info = {}
                info['gain'] = - (total_loss*10*40+0.1*np.sum(np.round(np.abs(action-self.state[1:4])*self.base))) # bus4:10*40;bus13:5*40
                # info['cost2'] = 1 * (np.sum(Vi < 0.95) + np.sum(Vi > 1.05))
                # info['cost'] = 1 * (
                #         np.sum(local_sigmoid(0.95, Vi, 200) * (Vi <= 1) + local_sigmoid(Vi, 1.05, 200) * (Vi > 1)))
