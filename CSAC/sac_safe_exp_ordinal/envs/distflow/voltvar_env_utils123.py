...
def load_flow(action,load):
    ...
    vref = 1+action[0]*0.05
    tsf_pos = {10: action[1]*0.05+1,  # from node 10 to node 15 line[10]
               24: action[2]*0.05+1,  # from node 26 to node 27 line[24]
               116: action[3]*0.05+1  # from node 61 to node 68 line[116]
    }
    cap_pos = {83: 0.02+0.02*action[4],   # 200KVA, base 5MVA, at node 84/node[83]
               88: 0.005+0.005*action[5],   # 50KVA, base 5MVA, at node 89/node[88]
               90: 0.005+0.005*action[6],   # 50KVA, base 5MVA, at node 91/node[90]
               92: 0.005+0.005*action[7]   # 50KVA, base 5MVA, at node 93/node[92]
    }
