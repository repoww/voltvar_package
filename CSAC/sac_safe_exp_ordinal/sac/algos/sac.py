...
class SAC(RLAlgorithm, Serializable):
    ...
    def _init_critic_update(self):
        ...
        ycs = tf.stop_gradient(
            self.scale_reward * self._costs_ph +
        ...
        self._tdc_loss1_t = 0.5 * tf.reduce_mean((ycs - self._qfc1_t) ** 2)
        self._tdc_loss2_t = 0.5 * tf.reduce_mean((ycs - self._qfc2_t) ** 2)
    ...
    def _init_actor_update(self):
        ...
        self._vfc_loss_t = 0.5 * tf.reduce_mean((
            self._vfc_t
            - tf.stop_gradient(min_log_targetc - log_pi + policy_prior_log_probs)
        ) ** 2)
