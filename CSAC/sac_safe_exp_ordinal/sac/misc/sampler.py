...
def rollout(env, policy, path_length, week, render=False, speedup=None):
    Cn = env.action_space.comp_num
    Do = env.observation_space.flat_dim

    env.wrapped_env.mode = 'Test'  ###
    env.wrapped_env.week = week  ###
    observation = env.reset()
    policy.reset()

    observations = np.zeros((path_length + 1, Do))
    actions = np.zeros((path_length, Cn))
    terminals = np.zeros((path_length, ))
    rewards = np.zeros((path_length, ))
    costs = np.zeros((path_length,))
    agent_infos = []
    env_infos = []
...
def rollouts(env, policy, path_length, n_paths):
    paths = [
        rollout(env, policy, path_length, week)
        for week in range(n_paths)
    ]
