...
class UniformPolicy2(Policy, Serializable):
    """
    Fixed policy that randomly samples actions uniformly at random.
    ...
    def __init__(self, env_spec):
        ...
        super(UniformPolicy2, self).__init__(env_spec)

    # Assumes action spaces are normalized to be the interval [-1, 1]
    @overrides
    def get_action(self, observation):
        return [(np.random.randint(d)-b)/b for d,b in zip(self._Da,self._Ba)], None
