...
from sac.misc.mlp import MLPFunction,MLPFunction2
...
class NNQFunction2(MLPFunction2):
    def __init__(self, env_spec, hidden_layer_sizes=(100, 100), name='qf'):
        Serializable.quick_init(self, locals())
        ...
        #     name='actions',
        # )
        super(NNQFunction2, self).__init__(
             name, self._obs_pl, hidden_layer_sizes,output_size=self._Da)
        # super(NNQFunction2, self).__init__(
        #     name, (self._obs_pl, self._action_pl), hidden_layer_sizes,output_size=self._Da)
