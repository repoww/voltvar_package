...
from .dqn_base import RLAlgorithm
...
class DQN(RLAlgorithm, Serializable):
    ...
    def __init__(
        ...
        super(DQN, self).__init__(**base_kwargs)
    ...
    def _init_Q_update(self):
        ...
        self._qf_t_all = self._qf.get_output_for(
            self._observations_ph,  reuse=True)  # N

        act_v = tf.one_hot(self._actions_ph, self._Da)
        self._qf_t = tf.reduce_sum(self._qf_t_all * act_v, 1)

        self._qftarg_params = self._qftarg.get_params_internal()

        self._qftarg_nextall_t = self._qftarg.get_output_for(
            self._next_observations_ph, reuse=True)  # N


        ys = tf.stop_gradient(
             self._rewards_ph +
            (1 - self._terminals_ph) * self._discount * tf.reduce_max(self._qftarg_nextall_t, axis=1)
        )  # N

        # qf_regularization_losses = tf.nn.l2_loss(self._qf_params)
        qf_regularization_losses = tf.get_collection(
            tf.GraphKeys.REGULARIZATION_LOSSES,
            scope=self._qf.name)

        qf_regularization_loss = tf.reduce_sum(
            qf_regularization_losses)

        self._td_loss_t = 0.5 * tf.reduce_mean((ys - self._qf_t)**2) + qf_regularization_loss
    ...
    def _init_target_ops(self):
        ...
        target_params = self._qftarg_params
    ...
    def _init_training(self, env, policy, pool):
        super(DQN, self)._init_training(env, policy, pool)
    ...
    def _do_training(self, iteration, batch):
        ...
        if iteration > 0 and iteration % self._target_update_interval == 0:
            # Run target ops here.
            self._sess.run(self._target_ops)
    ...
    def log_diagnostics(self, iteration, batch):
        ...
        qf, td_loss= self._sess.run(
            (self._qf_t, self._td_loss_t), feed_dict)
        ...
        logger.record_tabular('epsilon', self.policy._epsilon)
