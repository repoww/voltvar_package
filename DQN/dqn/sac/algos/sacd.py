...
class SACD(RLAlgorithm, Serializable):
    ...
    def _init_critic_update(self):
        ...
        _, self._qf1_t = self._qf1.get_output_for(
            self._observations_ph, self._actions_ph, reuse=True)  # N
        _, self._qf2_t = self._qf2.get_output_for(
            self._observations_ph, self._actions_ph, reuse=True)  # N
    ...
    def _init_actor_update(self):
        ...
        actions, logp_all, p_all, logp_pi = self._policy.actions_for(observations=self._observations_ph)
        ...
        log_target1_all, log_target1= self._qf1.get_output_for(
            self._observations_ph, actions, reuse=True)  # N
        log_target2_all, log_target2= self._qf2.get_output_for(
            self._observations_ph, actions, reuse=True)  # N
        ...
        policy_kl_loss = tf.reduce_mean(p_all * (logp_all - log_target1_all))
        ...
        self._vf_loss_t = 0.5 * tf.reduce_mean((
          self._vf_t
          - tf.stop_gradient(min_log_target - logp_pi + policy_prior_log_probs)
        )**2)
