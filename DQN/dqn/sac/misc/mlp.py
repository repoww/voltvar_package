...
class MLPFunction2(Parameterized, Serializable):

    def __init__(self, name, input_pls, hidden_layer_sizes,output_size,
                 output_nonlinearity=None):
        Parameterized.__init__(self)
        Serializable.quick_init(self, locals())
        ...
        self._layer_sizes = list(hidden_layer_sizes) + [output_size]
        self._output_nonlinearity = output_nonlinearity
        self._output_t = self.get_output_for(self._input_pls)
        #self._output_t = self.get_output_for(self._input_pls[0],self._input_pls[1])

    def get_output_for(self, input_s, reuse=False):
        ...
        with tf.variable_scope(self._name, reuse=reuse):
            ...
            #act_v = tf.one_hot(input_a, self._output_size)
            #value_t = tf.reduce_sum(value_all * act_v, 1)

            return value_all#, value_t


    def eval(self, inputs):
        feeds = {self._input_pls: inputs}
